...
export function useArchivedBatchPresenter() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      console.error('Error fetching archived batch orders:', error);
      setIsLoading(false);
    });
  ...
  const archivedOrders = useMemo(() => {
    return orders.filter(order => {
      ...
      // Ne garder QUE les commandes expédiées (FULFILLED)
  ...
  return {
    pendingOrders: sortedArchivedOrders,
    orderStats,
    isReversed,
    toggleOrder,
    selectedOrder,
